...
public class LevelController : MonoBehaviour {
    ...
    IEnumerator PowerupBonusCreation() 
    {
        while (true) 
        {
            ...
            // Tính toán vị trí spawn dựa trên viewport của camera
            ...
            Instantiate(
                powerUp,
                //Set the position for the new bonus: for X-axis - random position between the screen borders; for Y-axis - right above the upper screen border 
                new Vector2(
                    Random.Range(minScreenBounds.x, maxScreenBounds.x), 
                    maxScreenBounds.y + powerUp.GetComponent<Renderer>().bounds.size.y / 2), 
                Quaternion.identity
                );
        }
    }
